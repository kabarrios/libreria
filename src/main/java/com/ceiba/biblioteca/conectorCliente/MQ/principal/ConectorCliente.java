...
/**
 * Clase que permite la inteacci�n con Websphere MQ a travez de JMS.
 * <p>Provee de m�todos para el envio de mensajes (JMSTextMessage) s�ncronos y as�ncronos a colas de MQ.</p>
 ...
 *		"01", "01", "06", "01",
		"12051999", "192.168.133.100", 
		"4", "<datos_negocio/>", 
		60, "APPLICACION", map, selector);</pre>
 ...
 * @author echicas
 */
public class ConectorCliente extends ConectorClienteBase implements Serializable {
	 ...
	 */
	public ConectorCliente(String jndiResource){
		try{
			this.initParameters();
	...
	public ConectorCliente(){
		try{
			...
			this.context.initConnection(configuracionMQ.getQueueManager(), new URL(configuracionMQ.getCcdtPath()), configuracionMQ.getClientReconnectTimeout());
	 ...
	 * @throws Exception Si se produce error al cargar los parametros para la conexi�n con Websphere MQ
	 */
	public ConectorCliente(QueueConnectionFactory mqConnFactory){
		try{
			this.initParameters();
			this.context = new JMSQueueManagerContextjndi();
	...
	//	ConectorClienteBase conector = new ConectorClienteBase();
		ConectorClienteBase conector = null;
		try {
		//	conector = new ConectorCliente();
	...
	public static void sendAsynchronousMessageToESB(String nombreJndi, String servicio, String pais, String compania, String cadena, String sucursal, String numeroTransaccion, String hostOrigen, String token, String datos, String nombreAplicacion, Map jmsUserProperties){
		ConectorClienteBase conector = null;
		try {
			conector = new ConectorCliente(nombreJndi);
			sendAsynchronousMessageToESB(conector, servicio, pais, compania, cadena, sucursal, numeroTransaccion, hostOrigen, token, datos, nombreAplicacion, jmsUserProperties, null);
		} 
		catch (Exception e) {
			throw new RuntimeException(e);
